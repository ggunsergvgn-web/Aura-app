...
const fetchProf=async id=>{if(pCache.has(id))return pCache.get(id);const{data}=await supabase.from("profiles").select("username,avatar_url").eq("id",id).single();if(data)pCache.set(id,data);return data;};
...
function menuItem(color){return{display:"block",width:"100%",padding:"11px 16px",background:"none",border:"none",color,cursor:"pointer",textAlign:"left",fontSize:13};}
...
function PostScreen({user,profile,onPosted,accent}){
  ...
  const loadMyPosts=useCallback(async()=>{
    const{data}=await supabase.from("posts").select("*").eq("user_id",user.id).order("created_at",{ascending:false}).limit(20);
    if(!data){setLoadingPosts(false);return;}
    const withP=await Promise.all(data.map(async p=>{const prof=await fetchProf(p.user_id);return{...p,profiles:prof};}));
    setMyPosts(withP);setLoadingPosts(false);
  },[user]);
  useEffect(()=>{loadMyPosts();},[loadMyPosts]);
  const getLocation=()=>{
    if(!navigator.geolocation)return;setGettingLoc(true);
    navigator.geolocation.getCurrentPosition(async pos=>{
      try{const r=await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${pos.coords.latitude}&lon=${pos.coords.longitude}&format=json`);const d=await r.json();setLocation(d.address?.city||d.address?.town||"Konum alındı");}
  ...
  const gBtn=(active,col=accent)=>({background:active?`${col}18`:"rgba(255,255,255,0.04)",border:`1px solid ${active?col+"44":"rgba(255,255,255,0.08)"}`,borderRadius:20,padding:"6px 12px",color:active?col:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:13});
  return(
      ...
      <div style={{borderRadius:20,background:bgColor!=="transparent"?bgColor:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",padding:16,marginBottom:14}}>
        ...
        {mediaPreview&&(
          <div style={{position:"relative",marginBottom:10,borderRadius:12,overflow:"hidden"}}>
            {mediaType==="video"?<video src={mediaPreview} style={{width:"100%",maxHeight:240}} controls/>:<img src={mediaPreview} alt="" style={{width:"100%",maxHeight:240,objectFit:"cover"}}/>}
        ...
        <div style={{display:"flex",gap:8,borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:12,flexWrap:"wrap"}}>
          <button onClick={()=>fileRef.current.click()} style={gBtn(!!mediaFile)}>📷 Fotoğraf</button>
          <input ref={fileRef} type="file" accept="image/*,.heic,.heif,video/*" onChange={handleFile} style={{display:"none"}}/>
          <button onClick={()=>{setShowMoods(!showMoods);setShowBgs(false);}} style={gBtn(!!mood)}>{mood||"😊"} Mood</button>
          <button onClick={()=>{setShowBgs(!showBgs);setShowMoods(false);}} style={gBtn(bgColor!=="transparent")}>🎨</button>
          <button onClick={getLocation} style={gBtn(!!location)}>📍</button>
        </div>
      </div>
...
function FeedScreen({user,profile,accent,onThemeChange}){
  ...
  const loadPosts=useCallback(async()=>{
    ...
    setPosts([...pinned,...rest]);setLoading(false);
  },[user]);
  useEffect(()=>{
    loadPosts();
  ...
  const gBtn=(active,col)=>{col=col||accent;return{background:active?(col+"18"):"rgba(255,255,255,0.04)",border:"1px solid "+(active?(col+"44"):"rgba(255,255,255,0.08)"),borderRadius:20,padding:"6px 12px",color:active?col:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:13};};
  return(
    <div style={{background:"#050508"}}>
      <div style={{margin:"12px 14px",borderRadius:20,background:bgColor!=="transparent"?bgColor:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",padding:14}}>
        <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
          <Avatar name={profile&&profile.username} url={profile&&profile.avatar_url} size={40}/>
          <textarea value={content} onChange={e=>setContent(e.target.value)} placeholder={"Ne dusunuyorsun? "+(mood||"")} rows={2} style={{flex:1,background:"none",border:"none",outline:"none",color:"rgba(255,255,255,0.7)",fontSize:14,resize:"none",lineHeight:1.5,paddingTop:8}}/>
        </div>
        {(location||gettingLoc)&&<div style={{display:"flex",alignItems:"center",gap:6,marginTop:6,color:"#39ff14",fontSize:12}}>{"📍 "+(gettingLoc?"Konum aliniyor...":location)}{location&&<button onClick={()=>setLocation(null)} style={{background:"none",border:"none",color:"#ff6b9d",cursor:"pointer",fontSize:15}}>x</button>}</div>}
        {mediaPreview&&(
          <div style={{position:"relative",marginTop:10,borderRadius:12,overflow:"hidden"}}>
            ...
            <button onClick={()=>{setMediaFile(null);setMediaPreview(null);}} style={{position:"absolute",top:8,right:8,background:"rgba(0,0,0,0.6)",border:"none",borderRadius:"50%",width:28,height:28,color:"#fff",cursor:"pointer",fontSize:16}}>x</button>
          </div>
        )}
        ...
        <div style={{display:"flex",gap:6,justifyContent:"space-between",alignItems:"center",marginTop:10}}>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <button onClick={()=>fileRef.current.click()} style={gBtn(!!mediaFile)}>Foto</button>
            <input ref={fileRef} type="file" accept="image/*,.heic,.heif,video/*" onChange={handleFile} style={{display:"none"}}/>
            <button onClick={()=>{setShowMoods(!showMoods);setShowBgs(false);setShowTheme(false);}} style={gBtn(!!mood)}>{mood||"Mood"}</button>
            <button onClick={()=>{setShowBgs(!showBgs);setShowMoods(false);setShowTheme(false);}} style={gBtn(bgColor!=="transparent")}>Renk</button>
            <button onClick={()=>{setShowTheme(!showTheme);setShowMoods(false);setShowBgs(false);}} style={gBtn(false)}>Tema</button>
            <button onClick={getLocation} style={gBtn(!!location)}>Konum</button>
          </div>
      ...
      {trendPost&&(
        <div style={{margin:"0 14px 12px",borderRadius:16,background:"rgba(255,45,120,0.06)",border:"1px solid rgba(255,45,120,0.2)",padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:18}}>Trend</span>
          <div style={{flex:1,minWidth:0}}>
            <div style={{color:"rgba(255,45,120,0.8)",fontSize:10,fontWeight:700,letterSpacing:1,marginBottom:2}}>SON 1 SAATIN TRENDI</div>
            <div style={{color:"rgba(255,255,255,0.7)",fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(trendPost.profiles&&trendPost.profiles.username)+": "+(trendPost.content||"Medya")}</div>
          </div>
          <div style={{color:"#ff2d78",fontSize:12,fontWeight:700}}>{(trendPost.likes||[]).length+" begen"}</div>
        </div>
      )}
      ...
      {loading&&<div style={{textAlign:"center",padding:40,color:"rgba(255,255,255,0.3)"}}>Yukleniyor...</div>}
      {!loading&&posts.length===0&&<div style={{textAlign:"center",padding:60}}><div style={{color:"rgba(255,255,255,0.5)",fontSize:14}}>Henuz gonderi yok</div></div>}
      {posts.map(p=>(
        <PostCard key={p.id} post={p} user={user} accent={accent}
          onDelete={id=>setPosts(prev=>prev.filter(x=>x.id!==id))}
          onUpdate={(id,c)=>setPosts(prev=>prev.map(x=>x.id===id?Object.assign({},x,{content:c}):x))}/>
      ))}
    </div>
  );
}
...
function ExploreScreen({user,allProfiles,accent}){
  ...
  const toggleFollow=async targetId=>{
    if(followingIds.includes(targetId)){await supabase.from("follows").delete().eq("follower_id",user.id).eq("following_id",targetId);setFollowingIds(prev=>prev.filter(id=>id!==targetId));}
    else{await supabase.from("follows").insert({follower_id:user.id,following_id:targetId});setFollowingIds(prev=>[...prev,targetId]);}
  };
  const filtered=useMemo(()=>allProfiles.filter(u=>u.id!==user.id&&(u.username||"").toLowerCase().includes(search.toLowerCase())&&search.length>0),[allProfiles,search,user]);
  return(
    <div style={{background:"#050508",minHeight:"100%"}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:300,display:"flex",flexDirection:"column",backdropFilter:"blur(12px)"}} onClick={()=>setSelectedPost(null)}>
          <div style={{background:"rgba(10,10,20,0.98)",flex:1,overflowY:"auto",borderRadius:"24px 24px 0 0",marginTop:60,border:"1px solid rgba(255,255,255,0.08)",animation:"slideUp 0.3s ease"}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:16,display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
              ...
              <Avatar name={selectedPost.profiles?.username} url={selectedPost.profiles?.avatar_url} size={36}/>
              <div style={{color:"#fff",fontWeight:700}}>{selectedPost.profiles?.username}</div>
            </div>
            <PostCard post={selectedPost} user={user} accent={accent}
      ...
      <div style={{padding:"12px 16px 8px",position:"sticky",top:0,background:"rgba(5,5,8,0.92)",backdropFilter:"blur(20px)",zIndex:5,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"11px 14px"}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Kullanıcı ara..." style={{background:"none",border:"none",outline:"none",color:"#fff",fontSize:14,flex:1}}/>
        </div>
      </div>
      {search.length>0?(
        filtered.length===0
          ...
          :filtered.map(u=>(
            <div key={u.id} style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:14,borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
              ...
              <button onClick={()=>toggleFollow(u.id)} style={{background:followingIds.includes(u.id)?`${accent}18`:"rgba(255,255,255,0.04)",border:`1px solid ${followingIds.includes(u.id)?accent+"44":"rgba(255,255,255,0.08)"}`,borderRadius:20,padding:"7px 16px",color:followingIds.includes(u.id)?accent:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:13}}>
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:2,padding:2}}>
            {posts.map(p=>(
              <div key={p.id} style={{aspectRatio:"1",overflow:"hidden",position:"relative",cursor:"pointer"}} onClick={()=>setSelectedPost(p)}>
...
function ExploreScreen({user,allProfiles,accent}){
  ...
  const [selectedPost,setSelectedPost]=useState(null);
  useEffect(()=>{
    supabase.from("follows").select("following_id").eq("follower_id",user.id).then(function(r){if(r.data)setFollowingIds(r.data.map(function(f){return f.following_id;}));});
    var load=async function(){
      var r=await supabase.from("posts").select("*").not("media_url","is",null).order("created_at",{ascending:false}).limit(30);
      if(!r.data)return;
      var withP=await Promise.all(r.data.map(async function(p){var prof=await fetchProf(p.user_id);return Object.assign({},p,{profiles:prof});}));
      setPosts(withP);
    };
    load();
  },[user]);
  var toggleFollow=async function(targetId){
    if(followingIds.includes(targetId)){
      await supabase.from("follows").delete().eq("follower_id",user.id).eq("following_id",targetId);
      setFollowingIds(function(prev){return prev.filter(function(id){return id!==targetId;});});
    }else{
      await supabase.from("follows").insert({follower_id:user.id,following_id:targetId});
      setFollowingIds(function(prev){return prev.concat([targetId]);});
    }
  };
  var filtered=useMemo(function(){return allProfiles.filter(function(u){return u.id!==user.id&&(u.username||"").toLowerCase().includes(search.toLowerCase())&&search.length>0;});},[allProfiles,search,user]);
  return(
    <div style={{background:"#050508",minHeight:"100%"}}>
      {selectedPost&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:300,display:"flex",flexDirection:"column",backdropFilter:"blur(12px)"}} onClick={()=>setSelectedPost(null)}>
          <div style={{background:"rgba(10,10,20,0.98)",flex:1,overflowY:"auto",borderRadius:"24px 24px 0 0",marginTop:60,border:"1px solid rgba(255,255,255,0.08)",animation:"slideUp 0.3s ease"}} onClick={function(e){e.stopPropagation();}}>
            <div style={{padding:16,display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
              <button onClick={()=>setSelectedPost(null)} style={{background:"rgba(255,255,255,0.06)",border:"none",cursor:"pointer",color:accent,width:36,height:36,borderRadius:"50%",fontSize:20}}>&#8592;</button>
              <Avatar name={selectedPost.profiles&&selectedPost.profiles.username} url={selectedPost.profiles&&selectedPost.profiles.avatar_url} size={36}/>
              <div style={{color:"#fff",fontWeight:700}}>{selectedPost.profiles&&selectedPost.profiles.username}</div>
            </div>
            <PostCard post={selectedPost} user={user} accent={accent}
              onDelete={function(id){setPosts(function(prev){return prev.filter(function(x){return x.id!==id;});});setSelectedPost(null);}}
              onUpdate={function(id,c){setPosts(function(prev){return prev.map(function(x){return x.id===id?Object.assign({},x,{content:c}):x;});});}}/>
          </div>
        </div>
      )}
      <div style={{padding:"12px 16px 8px",position:"sticky",top:0,background:"rgba(5,5,8,0.92)",backdropFilter:"blur(20px)",zIndex:5,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"11px 14px"}}>
          <input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder="Kullanici ara..." style={{background:"none",border:"none",outline:"none",color:"#fff",fontSize:14,flex:1}}/>
        </div>
      </div>
      {search.length>0?(
        filtered.length===0
          ?<div style={{textAlign:"center",padding:40,color:"rgba(255,255,255,0.3)"}}>Kullanici bulunamadi</div>
          :filtered.map(function(u){return(
            <div key={u.id} style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:14,borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
              ...
              <button onClick={function(){toggleFollow(u.id);}} style={{background:followingIds.includes(u.id)?(accent+"18"):"rgba(255,255,255,0.04)",border:"1px solid "+(followingIds.includes(u.id)?(accent+"44"):"rgba(255,255,255,0.08)"),borderRadius:20,padding:"7px 16px",color:followingIds.includes(u.id)?accent:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:13}}>
                {followingIds.includes(u.id)?"Takipte":"Takip Et"}
              </button>
            </div>
          );})
        ...
        <div>
          <div style={{padding:"14px 16px 6px",color:"rgba(255,255,255,0.25)",fontSize:10,fontWeight:700,letterSpacing:2}}>POPULER</div>
          {posts.length===0&&<div style={{textAlign:"center",padding:40,color:"rgba(255,255,255,0.3)"}}>Medya paylasimi yok</div>}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:2,padding:2}}>
            {posts.map(function(p){return(
              <div key={p.id} style={{aspectRatio:"1",overflow:"hidden",position:"relative",cursor:"pointer"}} onClick={function(){setSelectedPost(p);}}>
                {p.media_type==="video"?<video src={p.media_url} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<img src={p.media_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>}
                <div style={{position:"absolute",bottom:4,right:4,background:"rgba(0,0,0,0.6)",borderRadius:6,padding:"2px 6px",fontSize:10,color:"#fff"}}>{(p.likes||[]).length}</div>
              </div>
            );})}
          </div>
        </div>
...
function ProfileScreen({user,profile,onLogout,onUpdateProfile,accent}){
  ...
  const [activeTab,setActiveTab]=useState("posts");
  const [editing,setEditing]=useState(false);
  const [newUsername,setNewUsername]=useState((profile&&profile.username)||"");
  ...
  const [selectedPost,setSelectedPost]=useState(null);
  const avatarRef=useRef();
  useEffect(()=>{setNewUsername((profile&&profile.username)||"");setNewBio((profile&&profile.bio)||"");},[profile]);
  useEffect(()=>{
    Promise.all([
      supabase.from("posts").select("*").eq("user_id",user.id).order("created_at",{ascending:false}),
      supabase.from("follows").select("*").eq("following_id",user.id),
      ...
      var p=results[0].data,f1=results[1].data,f2=results[2].data,s=results[3].data;
      if(p)setPosts(p);if(f1)setFollowers(f1);if(f2)setFollowing(f2);
      if(s)setSavedPosts(s.map(function(x){return x.posts;}).filter(Boolean));
    });
  },[user]);
  var saveProfile=async function(){
    if(!newUsername.trim())return;setSaving(true);setError("");
    var ex=await supabase.from("profiles").select("id").eq("username",newUsername.trim()).neq("id",user.id).maybeSingle();
    if(ex.data){setError("Bu kullanici adi alinmis!");setSaving(false);return;}
    var r=await supabase.from("profiles").update({username:newUsername.trim(),bio:newBio.trim()}).eq("id",user.id);
    if(r.error){setError("Kaydetme hatasi!");setSaving(false);return;}
    var updated=Object.assign({},profile,{username:newUsername.trim(),bio:newBio.trim()});
    lsSet("aura_profile",updated);
    pCache.set(user.id,{username:updated.username,avatar_url:updated.avatar_url});
    onUpdateProfile(updated);
    setEditing(false);setSaving(false);
  };
  var uploadAvatar=async function(e){
    var raw=e.target.files[0];if(!raw)return;setUploadingAvatar(true);
    var f=await convertFile(raw);
    var ext=f.name.split(".").pop();
    var path="avatars/"+user.id+"_"+Date.now()+"."+ext;
    var r=await supabase.storage.from("post-media").upload(path,f,{upsert:true});
    if(r.error){setUploadingAvatar(false);return;}
    var pub=supabase.storage.from("post-media").getPublicUrl(path);
    var newUrl=pub.data.publicUrl+"?v="+Date.now();
    await supabase.from("profiles").update({avatar_url:newUrl}).eq("id",user.id);
    var updated=Object.assign({},profile,{avatar_url:newUrl});
    lsSet("aura_profile",updated);
    pCache.set(user.id,{username:profile&&profile.username,avatar_url:newUrl});
    onUpdateProfile(updated);
    setUploadingAvatar(false);
  };
  var allPosts=activeTab==="posts"?posts:savedPosts;
  var mediaPosts=allPosts.filter(function(p){return p&&p.media_url;});
  var textPosts=allPosts.filter(function(p){return p&&!p.media_url&&p.content;});
  return(
    <div style={{background:"#050508",minHeight:"100%"}}>
      {selectedPost&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:300,display:"flex",flexDirection:"column",backdropFilter:"blur(12px)"}} onClick={()=>setSelectedPost(null)}>
          <div style={{background:"rgba(10,10,20,0.98)",flex:1,overflowY:"auto",borderRadius:"24px 24px 0 0",marginTop:60,border:"1px solid rgba(255,255,255,0.08)",animation:"slideUp 0.3s ease"}} onClick={function(e){e.stopPropagation();}}>
            <div style={{padding:16,display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
              <button onClick={()=>setSelectedPost(null)} style={{background:"rgba(255,255,255,0.06)",border:"none",cursor:"pointer",color:accent,width:36,height:36,borderRadius:"50%",fontSize:20}}>&#8592;</button>
              <div style={{color:"#fff",fontWeight:700}}>Gonderi</div>
            </div>
            <PostCard post={Object.assign({},selectedPost,{profiles:{username:profile&&profile.username,avatar_url:profile&&profile.avatar_url}})} user={user} accent={accent}
              onDelete={function(id){setPosts(function(prev){return prev.filter(function(x){return x.id!==id;});});setSelectedPost(null);}}
              onUpdate={function(id,c){setPosts(function(prev){return prev.map(function(x){return x.id===id?Object.assign({},x,{content:c}):x;});});setSelectedPost(function(prev){return prev&&prev.id===id?Object.assign({},prev,{content:c}):prev;});}}/>
          </div>
        </div>
      )}
      <div style={{padding:"24px 20px 16px",display:"flex",flexDirection:"column",alignItems:"center",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        <div style={{position:"relative",marginBottom:16}}>
          <Avatar name={profile&&profile.username} url={profile&&profile.avatar_url} size={96}/>
          <button onClick={()=>avatarRef.current.click()} style={{position:"absolute",bottom:0,right:0,width:30,height:30,borderRadius:"50%",background:"linear-gradient(135deg,"+accent+",#bf5af2)",border:"2.5px solid #050508",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>
        ...
        {editing?(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8,width:"100%"}}>
            <input value={newUsername} onChange={function(e){setNewUsername(e.target.value);}} placeholder="Kullanici adi" style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"13px 16px",color:"#fff",fontSize:14,outline:"none",width:"80%",textAlign:"center"}}/>
            <textarea value={newBio} onChange={function(e){setNewBio(e.target.value);}} placeholder="Hakkinda..." rows={2} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"13px 16px",color:"#fff",fontSize:14,outline:"none",width:"80%",resize:"none"}}/>
            {error&&<div style={{color:"#ff6b9d",fontSize:12}}>{error}</div>}
            <div style={{display:"flex",gap:8}}>
              <button onClick={saveProfile} disabled={saving} style={{border:"none",borderRadius:14,background:"linear-gradient(135deg,"+accent+",#bf5af2)",color:"#fff",fontWeight:600,cursor:"pointer",padding:"8px 20px",fontSize:13}}>{saving?"...":"Kaydet"}</button>
              <button onClick={()=>setEditing(false)} style={{border:"1px solid rgba(255,255,255,0.1)",borderRadius:14,background:"rgba(255,255,255,0.06)",color:"#fff",cursor:"pointer",padding:"8px 20px",fontSize:13}}>Iptal</button>
            </div>
          </div>
        ):(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            <div style={{color:"#fff",fontSize:22,fontWeight:800}}>{(profile&&profile.username)||"Kullanici"}</div>
            <div style={{color:"rgba(255,255,255,0.3)",fontSize:13}}>{user&&user.email}</div>
            {profile&&profile.bio&&<div style={{color:"rgba(255,255,255,0.5)",fontSize:13,marginTop:8,textAlign:"center",maxWidth:280,lineHeight:1.5}}>{profile.bio}</div>}
            <button onClick={()=>setEditing(true)} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:20,padding:"8px 20px",color:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:13,marginTop:12}}>Profili Duzenle</button>
          </div>
        )}
        <div style={{display:"flex",gap:32,marginTop:20}}>
          {[[posts.length,"Gonderi"],[followers.length,"Takipci"],[following.length,"Takip"]].map(function(item){return(
            <div key={item[1]} style={{textAlign:"center"}}>
              <div style={{color:"#fff",fontSize:20,fontWeight:800}}>{item[0]}</div>
              <div style={{color:accent+"99",fontSize:12,marginTop:1}}>{item[1]}</div>
            </div>
          );})}
        </div>
        <button onClick={onLogout} style={{marginTop:16,padding:"9px 28px",borderRadius:20,background:"rgba(255,45,120,0.08)",border:"1px solid rgba(255,45,120,0.25)",color:"#ff6b9d",fontWeight:700,cursor:"pointer",fontSize:13}}>Cikis Yap</button>
      </div>
      <div style={{display:"flex",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
        {[["posts","Gonderiler"],["saved","Kaydedilenler"]].map(function(item){return(
          <button key={item[0]} onClick={function(){setActiveTab(item[0]);}} style={{flex:1,padding:12,background:"none",border:"none",borderBottom:activeTab===item[0]?("2px solid "+accent):"2px solid transparent",color:activeTab===item[0]?accent:"rgba(255,255,255,0.3)",cursor:"pointer",fontSize:12,fontWeight:600}}>{item[1]}</button>
        );})}
      </div>
      {mediaPosts.length>0&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:2,padding:2}}>
          {mediaPosts.map(function(p){return(
            <div key={p.id} style={{aspectRatio:"1",overflow:"hidden",cursor:"pointer",position:"relative"}} onClick={function(){setSelectedPost(p);}}>
              {p.media_type==="video"?<video src={p.media_url} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<img src={p.media_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>}
              {p.pinned&&<div style={{position:"absolute",top:4,right:4,background:"rgba(0,0,0,0.6)",borderRadius:6,padding:"2px 4px",fontSize:10}}>P</div>}
            </div>
          );})}
        </div>
      )}
      {textPosts.map(function(p){return(
        <div key={p.id} onClick={function(){setSelectedPost(p);}} style={{margin:"0 14px 10px",padding:"14px 16px",borderRadius:16,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",cursor:"pointer"}}>
          {p.mood&&<span style={{fontSize:18,marginRight:6}}>{p.mood}</span>}
          <span style={{color:"rgba(255,255,255,0.8)",fontSize:14,lineHeight:1.6}}>{p.content}</span>
          <div style={{color:"rgba(255,255,255,0.25)",fontSize:11,marginTop:8}}>{timeAgo(p.created_at)+" - "+(p.likes||[]).length+" begen"}</div>
        </div>
      );})}
      {allPosts.length===0&&(
        <div style={{textAlign:"center",padding:40}}>
          <div style={{color:"rgba(255,255,255,0.3)",fontSize:14}}>{activeTab==="saved"?"Kaydedilen yok":"Henuz paylasim yok"}</div>
        </div>
      )}
    </div>
  );
...
function MessagesScreen({user,profile,allProfiles,accent}){
  ...
  useEffect(()=>{
    var ch=supabase.channel("unread_g").on("postgres_changes",{event:"INSERT",schema:"public",table:"messages"},function(p){
      var msg=p.new;
      if(msg.receiver_id===user.id){setUnread(function(prev){var n=Object.assign({},prev);n[msg.sender_id]=(n[msg.sender_id]||0)+1;return n;});setLastMessages(function(prev){var n=Object.assign({},prev);n[msg.sender_id]=msg;return n;});}
      if(msg.sender_id===user.id){setLastMessages(function(prev){var n=Object.assign({},prev);n[msg.receiver_id]=msg;return n;});}
    }).subscribe();
    return function(){supabase.removeChannel(ch);};
  },[user]);
  useEffect(()=>{
    if(!selectedUser)return;
    setUnread(function(prev){var n=Object.assign({},prev);n[selectedUser.id]=0;return n;});
    supabase.from("messages").select("*").or("and(sender_id.eq."+user.id+",receiver_id.eq."+selectedUser.id+"),and(sender_id.eq."+selectedUser.id+",receiver_id.eq."+user.id+")").order("created_at").then(function(r){if(r.data)setMessages(r.data);});
    var ch=supabase.channel("chat_"+selectedUser.id).on("postgres_changes",{event:"INSERT",schema:"public",table:"messages"},function(p){
      var msg=p.new;
      if((msg.sender_id===user.id&&msg.receiver_id===selectedUser.id)||(msg.sender_id===selectedUser.id&&msg.receiver_id===user.id)){
        setMessages(function(prev){return prev.find(function(m){return m.id===msg.id;})?prev:prev.concat([msg]);});
      }
    }).subscribe();
    return function(){supabase.removeChannel(ch);};
  },[selectedUser,user]);
  useEffect(()=>{if(messagesEndRef.current)messagesEndRef.current.scrollIntoView({behavior:"smooth"});},[messages]);
  var sendMessage=async function(content,type){
    type=type||"text";
    var text=content||input.trim();if(!text||!selectedUser)return;
    var temp={id:"tmp_"+Date.now(),sender_id:user.id,receiver_id:selectedUser.id,content:text,type:type,created_at:new Date().toISOString()};
    setMessages(function(prev){return prev.concat([temp]);});if(!content)setInput("");
    setShowEmoji(false);
    await supabase.from("messages").insert({sender_id:user.id,receiver_id:selectedUser.id,content:text,type:type});
  };
  var sendFile=async function(e){
    var raw=e.target.files[0];if(!raw)return;
    var f=await convertFile(raw);
    var ext=f.name.split(".").pop();
    var path="messages/"+user.id+"/"+Date.now()+"."+ext;
    var r=await supabase.storage.from("post-media").upload(path,f);
    if(r.error)return;
    var pub=supabase.storage.from("post-media").getPublicUrl(path);
    sendMessage(pub.data.publicUrl,f.type.startsWith("video")?"video":"image");
  };
  var followedUsers=useMemo(function(){return allProfiles.filter(function(u){return u.id!==user.id&&followingIds.includes(u.id);});},[allProfiles,user,followingIds]);
  var filtered=useMemo(function(){return followedUsers.filter(function(u){return (u.username||u.email||"").toLowerCase().includes(search.toLowerCase());});},[followedUsers,search]);
  var sorted=useMemo(function(){return filtered.slice().sort(function(a,b){return ((lastMessages[b.id]&&lastMessages[b.id].created_at)||"").localeCompare((lastMessages[a.id]&&lastMessages[a.id].created_at)||"");});},[filtered,lastMessages]);
  var totalUnread=useMemo(function(){return Object.values(unread).reduce(function(a,b){return a+b;},0);},[unread]);
  var grouped=useMemo(function(){return messages.reduce(function(g,msg){var date=new Date(msg.created_at).toLocaleDateString("tr-TR",{day:"numeric",month:"long",year:"numeric"});if(!g[date])g[date]=[];g[date].push(msg);return g;},{});},[messages]);

  if(selectedUser) return(
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:"#050508"}}>
      <div style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:12,background:"rgba(5,5,8,0.95)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.06)",flexShrink:0}}>
        <button onClick={function(){setSelectedUser(null);setMessages([]);}} style={{width:36,height:36,borderRadius:"50%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,color:accent,fontSize:18}}>&#8592;</button>
        <Avatar name={selectedUser.username} url={selectedUser.avatar_url} size={40} online={true}/>
        <div style={{flex:1}}>
          <div style={{color:"#fff",fontWeight:700,fontSize:15}}>{selectedUser.username||selectedUser.email}</div>
          <div style={{color:"#39ff14",fontSize:11}}>cevrimici</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <div style={{width:36,height:36,borderRadius:"50%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:16}}>Tel</div>
          <div style={{width:36,height:36,borderRadius:"50%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:16}}>Vid</div>
        </div>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:4,scrollbarWidth:"none"}} onClick={function(){setCtxMenu(null);}}>
        {messages.length===0&&<div style={{textAlign:"center",padding:"40px 20px"}}><div style={{color:"rgba(255,255,255,0.4)",fontSize:14}}>Ilk mesaji sen at!</div></div>}
        {Object.entries(grouped).map(function(entry){
          var date=entry[0],msgs=entry[1];
          return(
            <div key={date}>
              <div style={{textAlign:"center",margin:"12px 0"}}><span style={{background:"rgba(255,255,255,0.06)",color:"rgba(255,255,255,0.3)",fontSize:11,padding:"4px 12px",borderRadius:20}}>{date}</span></div>
              {msgs.map(function(msg,i){
                var isMe=msg.sender_id===user.id;
                var isImg=msg.type==="image",isVid=msg.type==="video";
                return(
                  <div key={msg.id} style={{display:"flex",justifyContent:isMe?"flex-end":"flex-start",alignItems:"flex-end",gap:8,marginBottom:2}}
                    onContextMenu={function(e){e.preventDefault();setCtxMenu({msg:msg,x:e.clientX,y:e.clientY});}}>
                    {!isMe&&<Avatar name={selectedUser.username} url={selectedUser.avatar_url} size={28}/>}
                    <div style={{display:"flex",flexDirection:"column",maxWidth:"72%",alignItems:isMe?"flex-end":"flex-start"}}>
                      <div style={{padding:isImg||isVid?"4px":"10px 14px",borderRadius:isMe?"18px 18px 4px 18px":"18px 18px 18px 4px",background:isMe?("linear-gradient(135deg,"+accent+"cc,#7b2ff7)"):"rgba(255,255,255,0.07)",border:isMe?"none":"1px solid rgba(255,255,255,0.08)",color:"#F1F5F9",fontSize:14,lineHeight:1.5,opacity:msg.id&&msg.id.toString().startsWith("tmp_")?0.6:1,overflow:"hidden"}}>
                        {isImg&&<img src={msg.content} alt="" style={{width:"100%",maxWidth:220,borderRadius:14,display:"block"}}/>}
                        {isVid&&<video src={msg.content} controls style={{width:"100%",maxWidth:220,borderRadius:14}}/>}
                        {!isImg&&!isVid&&msg.content}
                      </div>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:4,marginTop:3,flexDirection:isMe?"row-reverse":"row"}}>
                        <span style={{fontSize:10,color:"rgba(255,255,255,0.25)"}}>{fmtTime(msg.created_at)}</span>
                        {isMe&&<span style={{fontSize:11,color:accent}}>vv</span>}
                      </div>
                    </div>
                  </div>
                );
      ...
      {ctxMenu&&(
        <div style={{position:"fixed",top:Math.min(ctxMenu.y,window.innerHeight-180),left:Math.min(ctxMenu.x,window.innerWidth-170),background:"rgba(15,15,25,0.98)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:6,zIndex:1000,minWidth:160,boxShadow:"0 16px 40px rgba(0,0,0,0.6)",backdropFilter:"blur(20px)"}}>
          <div style={{display:"flex",gap:4,padding:"8px 10px 4px",borderBottom:"1px solid rgba(255,255,255,0.06)",marginBottom:4}}>
            {QUICK_REACTIONS.map(function(em){return(<div key={em} style={{fontSize:22,cursor:"pointer",padding:4,borderRadius:8}} onClick={function(){setReactions(function(prev){var n=Object.assign({},prev);n[ctxMenu.msg.id]=em;return n;});setCtxMenu(null);}}>{em}</div>);})}
          </div>
          <div onClick={function(){navigator.clipboard&&navigator.clipboard.writeText(ctxMenu.msg.content);setCtxMenu(null);}} style={{padding:"10px 14px",borderRadius:10,cursor:"pointer",color:"#fff",fontSize:13}}>Kopyala</div>
          {ctxMenu.msg.sender_id===user.id&&<div onClick={async function(){setMessages(function(prev){return prev.filter(function(m){return m.id!==ctxMenu.msg.id;});});await supabase.from("messages").delete().eq("id",ctxMenu.msg.id);setCtxMenu(null);}} style={{padding:"10px 14px",borderRadius:10,cursor:"pointer",color:"#ff6b9d",fontSize:13}}>Sil</div>}
          <div onClick={function(){setCtxMenu(null);}} style={{padding:"10px 14px",borderRadius:10,cursor:"pointer",color:"rgba(255,255,255,0.4)",fontSize:13}}>Kapat</div>
        </div>
      )}
      {showEmoji&&(
        <div style={{padding:"0 12px 8px"}}>
          <div style={{background:"rgba(15,15,25,0.98)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:20,padding:12,display:"flex",flexWrap:"wrap",gap:4,maxHeight:160,overflowY:"auto"}}>
            {EMOJIS.map(function(em){return(<div key={em} style={{fontSize:24,cursor:"pointer",padding:4,borderRadius:8}} onClick={function(){setInput(function(p){return p+em;});setShowEmoji(false);if(textareaRef.current)textareaRef.current.focus();}}>{em}</div>);})}
          </div>
        </div>
      )}
      <div style={{padding:"10px 12px 32px",background:"rgba(5,5,8,0.95)",backdropFilter:"blur(20px)",borderTop:"1px solid rgba(255,255,255,0.06)",flexShrink:0}}>
        <input ref={fileRef} type="file" accept="image/*,.heic,.heif,video/*" onChange={sendFile} style={{display:"none"}}/>
        <div style={{display:"flex",alignItems:"flex-end",gap:8}}>
          <div onClick={function(){if(fileRef.current)fileRef.current.click();}} style={{width:40,height:40,borderRadius:"50%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:18,flexShrink:0}}>+</div>
          <div style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:22,padding:"11px 16px",display:"flex",alignItems:"center",gap:8}}>
            <textarea ref={textareaRef} value={input} onChange={function(e){setInput(e.target.value);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,100)+"px";}} onKeyDown={function(e){if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage();}}} placeholder="Mesaj yaz..." rows={1} style={{background:"none",border:"none",outline:"none",color:"#fff",fontSize:14,flex:1,resize:"none",maxHeight:100,overflowY:"auto",scrollbarWidth:"none",lineHeight:1.4}}/>
            <span onClick={function(){setShowEmoji(function(p){return !p;});}} style={{fontSize:18,cursor:"pointer",opacity:0.5,flexShrink:0}}>Em</span>
          </div>
          {input.trim()
            ?<button onClick={function(){sendMessage();}} style={{width:44,height:44,borderRadius:"50%",background:"linear-gradient(135deg,"+accent+",#bf5af2)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 4px 18px "+accent+"44",color:"#fff",fontSize:18}}>&#10148;</button>
            :<div style={{width:44,height:44,borderRadius:"50%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:18,flexShrink:0}}>Ses</div>
          }
        </div>
      </div>
    </div>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      <div style={{padding:"16px 16px 8px",background:"rgba(5,5,8,0.9)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.06)",flexShrink:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"11px 14px"}}>
          <input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder="Kisi ara..." style={{background:"none",border:"none",outline:"none",color:"#fff",fontSize:14,flex:1}}/>
        </div>
      </div>
      <div style={{flex:1,overflowY:"auto",scrollbarWidth:"none"}}>
        {sorted.length===0
          ?<div style={{textAlign:"center",padding:40}}><div style={{color:"rgba(255,255,255,0.3)",fontSize:14}}>Takip ettigin kisiler burada gorunur</div></div>
          :sorted.map(function(u){
            var last=lastMessages[u.id],uc=unread[u.id]||0,isU=uc>0;
            return(
              <div key={u.id} onClick={function(){setSelectedUser(u);}} style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,0.04)",background:isU?(accent+"08"):"transparent"}}>
                <Avatar name={u.username||u.email} url={u.avatar_url} size={52} online={true}/>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{color:"#fff",fontWeight:700,fontSize:14}}>{u.username||u.email}</div>
                  {last&&<div style={{color:isU?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.35)",fontSize:12,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                    {last.sender_id===user.id&&<span style={{color:accent+"88"}}>Sen: </span>}
                    {last.type==="image"?"Fotograf":last.type==="video"?"Video":(last.content||"").substring(0,40)}
                  </div>}
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5,flexShrink:0}}>
                  {last&&<div style={{color:isU?accent:"rgba(255,255,255,0.25)",fontSize:11}}>{fmtTime(last.created_at)}</div>}
                  {isU?<div style={{background:"linear-gradient(135deg,"+accent+",#bf5af2)",borderRadius:"50%",minWidth:20,height:20,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#fff",fontWeight:700,padding:"0 4px"}}>{uc}</div>:null}
                </div>
              </div>
            );
          })
