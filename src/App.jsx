...
function Avatar({ name, url, size = 42, online }) {
  ...
  return (
    <div style={{ position: "relative", flexShrink: 0 }}>
      <div style={{ width: size, height: size, borderRadius: "50%", background: url ? "transparent" : `linear-gradient(135deg, ${color}, ${color}99)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.32, fontWeight: 700, color: "#fff", border: "2px solid rgba(99,102,241,0.4)", overflow: "hidden" }}>
...
function AuthScreen({ onAuth }) {
  ...
  const inp = { background: "rgba(255,255,255,0.06)", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 16, padding: "14px 18px", color: "#1a1a2e", fontSize: 15, outline: "none", fontFamily: "inherit", width: "100%", boxSizing: "border-box", color: "#fff" };
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 28px", background: "linear-gradient(160deg, #0a0a1a 0%, #0d0d2b 50%, #0a0a1a 100%)" }}>
      <img src={LOGO} alt="Aura SC" style={{ width: 120, height: 120, borderRadius: 30, marginBottom: 20, objectFit: "cover", boxShadow: "0 0 60px rgba(99,102,241,0.5), 0 0 120px rgba(236,72,153,0.3)" }} />
      <div style={{ color: "#fff", fontSize: 32, fontWeight: 900, marginBottom: 4, letterSpacing: -1, background: "linear-gradient(90deg, #60A5FA, #A78BFA, #F472B6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>AURA SC</div>
      ...
      <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {success && <div style={{ color: "#60A5FA", fontSize: 13, textAlign: "center", padding: "8px", background: "rgba(96,165,250,0.1)", borderRadius: 10 }}>{success}</div>}
        <button onClick={handle} disabled={loading} style={{ background: "linear-gradient(135deg, #6366F1, #EC4899)", border: "none", borderRadius: 16, padding: "16px", color: "#fff", fontSize: 16, fontWeight: 800, cursor: "pointer", marginTop: 4, fontFamily: "inherit", boxShadow: "0 8px 30px rgba(99,102,241,0.4)" }}>
          {loading ? "⏳" : mode === "login" ? "Giriş Yap" : "Kayıt Ol"}
        </button>
        <button onClick={() => { setMode(mode === "login" ? "register" : "login"); setError(""); }} style={{ background: "none", border: "none", color: "rgba(148,163,184,0.7)", fontSize: 14, cursor: "pointer", fontFamily: "inherit", marginTop: 4 }}>
...
function MessagesScreen({ user, profile, allProfiles }) {
  ...
  const sendMessage = async () => {
    ...
    const tempMsg = { id: "temp_" + Date.now(), sender_id: user.id, receiver_id: selectedUser.id, content: input.trim(), created_at: new Date().toISOString() };
    setMessages(prev => [...prev, tempMsg]);
    setInput("");
    await supabase.from("messages").insert({ sender_id: user.id, receiver_id: selectedUser.id, content: tempMsg.content });
  };
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {selectedUser ? (
        <>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 10, background: "linear-gradient(180deg, #0d0d2b 0%, #0a0a1a 100%)" }}>
            ...
            {messages.map(msg => {
              ...
              return (
                <div key={msg.id} style={{ display: "flex", justifyContent: isMe ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 8 }}>
                  ...
                  <div style={{ maxWidth: "72%", padding: "10px 14px", borderRadius: isMe ? "18px 18px 4px 18px" : "18px 18px 18px 4px", background: isMe ? "linear-gradient(135deg, #6366F1, #EC4899)" : "rgba(255,255,255,0.06)", border: isMe ? "none" : "1px solid rgba(99,102,241,0.2)", color: "#F1F5F9", fontSize: 14, lineHeight: 1.5, opacity: msg.id?.startsWith("temp_") ? 0.7 : 1 }}>
                    {msg.content}
          ...
          {filtered.length === 0 ? (
            ...
            <div key={u.id} onClick={() => setSelectedUser(u)} style={{ padding: "12px 16px", display: "flex", alignItems: "center", gap: 14, cursor: "pointer", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: "rgba(129,140,248,0.6)", fontSize: 12, marginTop: 2 }}>Mesaj gönder →</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", background: "linear-gradient(180deg, #0d0d2b 0%, #0a0a1a 100%)" }}>
      <div style={{ padding: "12px 16px 8px", position: "sticky", top: 0, background: "rgba(10,10,26,0.95)", backdropFilter: "blur(20px)", zIndex: 5 }}>
        <div style={{ background: "rgba(99,102,241,0.08)", borderRadius: 20, padding: "10px 16px", display: "flex", gap: 8, alignItems: "center", border: "1px solid rgba(99,102,241,0.2)" }}>
          <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="rgba(129,140,248,0.8)" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Kullanıcı veya konu ara..." style={{ background: "none", border: "none", outline: "none", color: "#F1F5F9", fontSize: 13, width: "100%", fontFamily: "inherit" }} />
        </div>
      </div>
      {search.length > 0 ? (
        <div>
          {filtered.length === 0 ? <div style={{ textAlign: "center", color: "rgba(148,163,184,0.4)", padding: 30, fontSize: 14 }}>Kullanıcı bulunamadı</div> : filtered.map(u => (
        ...
        <div>
          ...
          {posts.length === 0 ? (
            <div style={{ textAlign: "center", color: "rgba(148,163,184,0.3)", padding: 40, fontSize: 14 }}>Henüz gönderi yok 🌀</div>
          ) : posts.map(post => (
            <div key={post.id} style={{ margin: "0 16px 12px", borderRadius: 20, overflow: "hidden", border: "1px solid rgba(99,102,241,0.15)", background: "rgba(99,102,241,0.05)" }}>
              ...
              <div style={{ padding: "12px 14px" }}>
                ...
                <div style={{ color: "#CBD5E1", fontSize: 14, lineHeight: 1.5 }}>{post.content}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ overflowY: "auto", flex: 1, background: "linear-gradient(180deg, #0d0d2b 0%, #0a0a1a 100%)" }}>
        <div style={{ margin: "12px 16px", borderRadius: 20, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.2)", padding: "14px" }}>
        ...
        {posts.map(post => {
          const liked = (post.likes || []).includes(user.id);
          return (
            <div key={post.id} style={{ margin: "0 16px 12px", borderRadius: 20, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(99,102,241,0.12)", overflow: "hidden" }}>
              ...
              <div style={{ padding: "10px 14px", display: "flex", gap: 20, borderTop: "1px solid rgba(99,102,241,0.08)" }}>
                <button onClick={() => toggleLike(post)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, color: liked ? "#EC4899" : "rgba(148,163,184,0.5)", fontSize: 13 }}>
                ...
                <button style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, color: "rgba(148,163,184,0.5)", fontSize: 13 }}>
    ...
    <div style={{ flex: 1, overflowY: "auto", background: "linear-gradient(180deg, #0d0d2b 0%, #0a0a1a 100%)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 2, padding: "2px" }}>
        ...
        {userPosts.filter(p => p.media_url).length === 0 && (
          <div style={{ gridColumn: "1/-1", textAlign: "center", color: "rgba(148,163,184,0.3)", padding: 40, fontSize: 14 }}>Henüz gönderi yok 🌀</div>
        )}
      </div>
    </div>
  );
