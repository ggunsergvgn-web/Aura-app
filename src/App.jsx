...
function Avatar({ name, color, size = 42, online }) {
  ...
  return (
    <div style={{ position: "relative", flexShrink: 0 }}>
      <div style={{ width: size, height: size, borderRadius: "50%", background: `linear-gradient(135deg, ${color || "#9B59B6"}, ${color || "#9B59B6"}88)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.3, fontWeight: 700, color: "#fff", border: "2px solid rgba(255,255,255,0.15)" }}>
        {initials}
      </div>
  ...
  const handle = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        const existing = await supabase.from("profiles").select("id").eq("username", username.trim()).single();
        if (existing.data) throw new Error("Bu kullanıcı adı alınmış!");
        const { data, error } = await supabase.auth.signUp({ email, password });
        if (error) throw error;
        if (data.user) {
          await supabase.from("profiles").upsert({ id: data.user.id, email, username: username.trim() });
        }
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 30px", background: "radial-gradient(ellipse at top, #2d0b5e 0%, #1a0533 50%, #0d0018 100%)" }}>
      ...
      <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
        {mode === "register" && (
          <input value={username} onChange={e => setUsername(e.target.value)} placeholder="Kullanıcı adı" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(168,85,247,0.3)", borderRadius: 16, padding: "14px 18px", color: "#fff", fontSize: 15, outline: "none", fontFamily: "inherit" }} />
        )}
...
function MessagesScreen({ user, profile }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("messages_" + selectedUser.id).on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
      ...
      {selectedUser ? (
        <>
          <div style={{ padding: "12px 20px", display: "flex", alignItems: "center", gap: 12, background: "rgba(168,85,247,0.1)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(168,85,247,0.2)" }}>
            ...
            <Avatar name={selectedUser.username || selectedUser.email || "?"} color="#9B59B6" size={38} />
            <div>
        ...
        <div style={{ flex: 1, overflowY: "auto", background: "linear-gradient(180deg, #1a0533 0%, #0d0018 100%)" }}>
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", padding: 40, fontSize: 14 }}>Henüz başka kullanıcı yok 😊</div>
          ) : filtered.map(u => (
            <div key={u.id} onClick={() => setSelectedUser(u)} style={{ padding: "12px 20px", display: "flex", alignItems: "center", gap: 14, cursor: "pointer", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
              <Avatar name={u.username || u.email || "?"} color="#9B59B6" size={50} />
              <div style={{ flex: 1 }}>
...
function ProfileScreen({ user, profile, onLogout, onUpdateUsername }) {
  ...
  const saveUsername = async () => {
    ...
    const existing = await supabase.from("profiles").select("id").eq("username", newUsername.trim()).neq("id", user.id).single();
    if (existing.data) { setError("Bu kullanıcı adı alınmış!"); setSaving(false); return; }
      ...
      <div style={{ padding: "20px", display: "flex", flexDirection: "column", alignItems: "center", borderBottom: "1px solid rgba(168,85,247,0.2)" }}>
        ...
        {editing ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, width: "100%" }}>
            <input value={newUsername} onChange={e => setNewUsername(e.target.value)} placeholder="Yeni kullanıcı adı" style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(168,85,247,0.3)", borderRadius: 12, padding: "10px 16px", color: "#fff", fontSize: 15, outline: "none", fontFamily: "inherit", textAlign: "center", width: "80%" }} />
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => setEditing(false)} style={{ padding: "8px 20px", borderRadius: 12, background: "rgba(255,255,255,0.08)", border: "none", color: "#fff", cursor: "pointer", fontFamily: "inherit" }}>İptal</button>
            </div>
          </div>
        ) : (
          <>
            <div style={{ color: "#fff", fontSize: 20, fontWeight: 800 }}>@{profile?.username || user?.email?.split("@")[0]}</div>
            <div style={{ color: "rgba(168,85,247,0.8)", fontSize: 13, marginTop: 4 }}>{user?.email}</div>
            <button onClick={() => setEditing(true)} style={{ marginTop: 12, padding: "8px 24px", borderRadius: 20, background: "rgba(168,85,247,0.2)", border: "1px solid rgba(168,85,247,0.4)", color: "#A855F7", fontWeight: 700, cursor: "pointer", fontSize: 13, fontFamily: "inherit" }}>Kullanıcı adını değiştir</button>
          </>
        )}
        ...
        <button onClick={onLogout} style={{ marginTop: 16, padding: "10px 32px", borderRadius: 20, background: "rgba(255,0,0,0.1)", border: "1px solid rgba(255,0,0,0.3)", color: "#F87171", fontWeight: 700, cursor: "pointer", fontSize: 14, fontFamily: "inherit" }}>Çıkış Yap</button>
      </div>
    ...
    <div style={{ fontFamily: "'Sora', sans-serif", background: "#0d0018", minHeight: "100vh", display: "flex", justifyContent: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: 430, height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "50px 20px 12px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(13,0,24,0.8)", backdropFilter: "blur(20px)" }}>
          ...
          {profile && <div style={{ color: "rgba(168,85,247,0.8)", fontSize: 13, fontWeight: 600 }}>@{profile.username || user?.email?.split("@")[0]}</div>}
        </div>
        ...
        {!user ? (
          ...
          <>
            {activeTab === "messages" && <MessagesScreen user={user} profile={profile} />}
            {activeTab === "feed" && <FeedScreen />}
            {activeTab === "aura" && <AuraZoneScreen />}
            {activeTab === "profile" && <ProfileScreen user={user} profile={profile} onLogout={() => { supabase.auth.signOut(); setUser(null); setProfile(null); }} onUpdateUsername={(u) => setProfile(p => ({ ...p, username: u }))} />}
            <div style={{ padding: "10px 20px 34px", display: "flex", justifyContent: "space-around", background: "rgba(13,0,24,0.9)", backdropFilter: "blur(20px)", borderTop: "1px solid rgba(168,85,247,0.15)" }}>
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, color: activeTab === tab.id ? "#A855F7" : "rgba(255,255,255,0.35)" }}>
                  {tab.icon}
