    ...
    <div className="msgs-wrap">
      ...
      <div className="msgs-list">
        {sorted.length === 0 ? (
          ...
          return (
            <div key={u.id} className={`conv-item ${isUnread ? "unread" : ""}`} onClick={() => setSelectedUser(u)}>
              <div className="conv-avatar-wrap">
                <Avatar name={u.username} url={u.avatar_url} size={52} online />
              </div>
              <div className="conv-body">
                <div className="conv-name">{u.username || u.email}</div>
                <div className={`conv-preview ${isUnread ? "unread-text" : ""}`}>
                  {last ? (
                    <>
              ...
              <div className="conv-meta">
                {last && <div className={`conv-time ${isUnread ? "unread-time" : ""}`}>{formatMsgTime(last.created_at)}</div>}
                {isUnread ? <div className="conv-badge">{unreadCount}</div> : last?.sender_id === user.id ? <span className="msg-ticks read">✓✓</span> : null}
              </div>
            </div>
          );
        })}
...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    const ch = supabase.channel("app_msgs")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
        if (payload.new.receiver_id === user.id && activeTab !== "messages") {
    ...
    <>
      ...
      <div className="aura-root">
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
          <style>{`@keyframes glow{0%,100%{box-shadow:0 0 40px rgba(0,245,255,0.4)}50%{box-shadow:0 0 70px rgba(0,245,255,0.7),0 0 120px rgba(191,90,242,0.4)}}`}</style>
          <img src={LOGO} style={{ width: 88, height: 88, borderRadius: 22, objectFit: "cover", animation: "glow 2s ease-in-out infinite" }} alt="" />
          ...
          <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 12, letterSpacing: 2 }}>Yükleniyor...</div>
        </div>
      </div>
    </>
  );
